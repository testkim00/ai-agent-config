---
name: env-sync
description: Claude ↔ Codex 간 환경변수(.env) 동기화 Claude의 `/env:sync` 명령을 Codex에서 skill로 사용할 때 대응 매핑으로 사용한다.
---
...
- Claude ↔ Codex 간 환경변수(.env) 동기화
- Claude의 `/env:sync`를 Codex에서 같은 의도로 수행해야 할 때

## source mapping

- Claude command: `/env:sync`
- Source file: `claude/commands/env/sync.md`
