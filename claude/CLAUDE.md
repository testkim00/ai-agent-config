...
> **⚠️ 오케스트레이션 시작 시 반드시:**
> 1. `~/.claude/skills/subagent-convention/_index.md` 확인
> 2. `instruction.md` 읽기 (5단계 흐름, 완료 조건)
> 3. 1단계부터 시간 기록 시작
> 4. **5단계 실행 분석 보고 완료 전까지 미완료**
...
**흐름:** `1단계(요청분석) → 2단계(Setter) → 3단계(계획수립) → 4단계(Executor) → 5단계(분석보고)`
