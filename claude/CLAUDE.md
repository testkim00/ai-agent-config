...
```
오케스트레이터 (Opus 직접) → 감독관 (Opus subagent) → 실행자 (Sonnet/Haiku)
                                    ↑
                              Codex (자문)
```
...
| 계층 | 모델 | 역할 |
|------|------|------|
| **오케스트레이터** | Opus (직접) | 전체 계획, 작업 분배, 최종 검토 |
| **감독관** | Opus (subagent) | 위임받은 작업 분석, 판단, 재위임 |
| **실행자** | Sonnet/Haiku | 단순 반복 작업 실행 |
| **자문** | Codex | 계획 검토, 전문 분야 자문 |
